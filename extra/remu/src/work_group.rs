...
impl<'a> WorkGroup<'a> {
    ...
    pub fn exec_waves(&mut self) -> Result<(), i32> {
        ...
        // Run waves until all reach s_endpgm, synchronizing at every barrier.
        // Each pass: run all waves until they hit a barrier or endpgm.
        // Repeat until all waves are done.
        loop {
            let mut any_hit_barrier = false;
            for w in waves.iter().enumerate() {
                match self.exec_wave(w) {
                    Ok(false) => {} // wave hit endpgm (done)
                    Ok(true) => any_hit_barrier = true, // wave hit barrier
                    Err(e) => return Err(e),
                }
            }
            if !any_hit_barrier {
    ...
    /// Execute a wave until it hits a barrier or endpgm.
    /// Returns Ok(true) if barrier hit, Ok(false) if endpgm reached.
    fn exec_wave(&mut self, (wave_id, threads): (usize, &&[[u32; 3]])) -> Result<bool, i32> {
        let (mut scalar_reg, mut scc, mut pc, mut vec_reg, mut vcc, mut exec, mut sds) = match self.wave_state.get(&wave_id) {
          None => {
            ...
            // AMDHSA convention: s[0:1]=kernarg_ptr, s2=TGID_X, s3=TGID_Y, s4=TGID_Z
            // (system SGPRs come right after user SGPRs)
            let [gx, gy, gz] = self.id;
            match self.dispatch_dim {
              3 => (scalar_reg[2], scalar_reg[3], scalar_reg[4]) = (gx, gy, gz),
              2 => (scalar_reg[2], scalar_reg[3]) = (gx, gy),
              _ => scalar_reg[2] = gx,
            }
        ...
        loop {
            if self.kernel[pc] == END_PRG {
                self.wave_state.remove(&wave_id);
                break Ok(false); // done
            }
            if self.kernel[pc] == S_BARRIER {
                // Save state and return — will resume after barrier sync
                pc += 1; // advance past the barrier instruction
                self.wave_state.insert(wave_id, WaveState { scalar_reg, scc, vec_reg: vec_reg.clone(), vcc: vcc.clone(), exec: exec.clone(), pc, sds: sds.clone() });
                break Ok(true); // hit barrier
            }
