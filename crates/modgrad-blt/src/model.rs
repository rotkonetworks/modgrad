...
impl BltModel {
    ...
    /// Generate `n_new_bytes` bytes with **partial amortization** of the
    /// encoder + latent stages across decoder steps.
    ///
    /// ## What this is — and what it isn't
    ///
    /// This is **not** true per-byte KV-caching. True KV-caching would
    /// require reaching into [`crate::encoder::LocalEncoder`] and
    /// [`crate::decoder::LocalDecoder`] to (a) skip their internal
    /// `byte_kv_cache.reset()` calls and (b) only iterate over the new
    /// byte positions per call. That touches encoder/decoder internals
    /// and is intentionally out of scope for this slice — see the
    /// `generate_cached` design notes in the BLT KV-cache TODO.
    ///
    /// What we DO here is a **boundary-keyed cache** of the post-latent
    /// patch reps (and the encoder's last-layer byte_reps used to seed
    /// the decoder). Each step computes the fixed-stride boundaries for
    /// the current `tokens.len()`. If the boundaries are bit-identical
    /// to the previously cached step's boundaries (i.e., the same bytes
    /// were partitioned the same way) AND the underlying tokens prefix
    /// is unchanged, we can reuse the cached `patch_reps_post_latent` +
    /// `seed_byte_reps` and skip stages 1+2. We then only run stage 3
    /// (the decoder).
    ///
    /// ## Why the speedup is modest under fixed-stride patching
    ///
    /// Under the fixed-stride patcher used by [`Self::generate`], every
    /// new generated byte either grows the last patch (boundaries[N-1]
    /// stays, boundaries[N] = n_bytes increments) or creates a new
    /// patch (when `stride` shifts), or falls in a fresh patch slot.
    /// In **every** step the boundaries change, so the cache hit rate
    /// is zero with the current patcher placeholder. The amortization
    /// shape is correct — it's the patcher that defeats it. A
    /// content-aware patcher (entropy patcher) that emits stable
    /// prefixes for stable byte prefixes would let this cache fire on
    /// the bulk of generation steps.
    ///
    /// We still keep this path because:
    ///   1. It demonstrates the decomposed forward shape (encoder →
    ///      latent → decoder split into independently callable
    ///      sub-paths, the prerequisite for any future KV-caching slice).
    ///   2. The greedy output **must** be bit-identical to
    ///      [`Self::generate`] — verified by the example. Any future
    ///      caching policy that produces lossy outputs will trip that
    ///      assertion immediately.
    ///   3. Out-of-scope: true incremental KV-caching is a future slice
    ///      that touches encoder/decoder internals. This method is the
    ///      structural scaffold it will plug into.
    ///
    /// ## Sampling + RNG semantics
    ///
    /// Identical to [`Self::generate`]: greedy if `temperature == 0.0`,
    /// otherwise softmax-temperature with the same Numerical Recipes
    /// LCG seeded by `rng_seed ^ step`. Same window-sliding behaviour
    /// when `tokens.len() > max_seq_len`.
    pub fn generate_cached(
        ...
        assert!(temperature >= 0.0,
            "BltModel::generate_cached: temperature must be non-negative, got {temperature}");

        let max_seq_len = self.config.encoder.max_seq_len;
        let max_patches = self.config.latent.max_patches;
        let patch_dim = self.config.latent.patch_dim;
        assert!(max_seq_len >= 2,
            "BltModel::generate_cached: encoder.max_seq_len must be >= 2 to support a window");
        assert!(max_patches >= 1,
            "BltModel::generate_cached: latent.max_patches must be >= 1");

        // Same LCG as `generate` — the determinism contract requires
        // bit-identical RNG draws across both code paths.
        #[inline]
        fn lcg_uniform(seed: u64, step: usize) -> f32 {
            let mut s = seed ^ ((step as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15));
            s = s.wrapping_mul(6364136223846793005)
        ...
        // ── Boundary-keyed cache state ───────────────────────────
        //
        // We cache:
        //   - `cached_boundaries`     — boundaries used to produce the
        //                               cached patch_reps and seed.
        //   - `cached_tokens_prefix`  — the bytes the cache was built
        //                               over. Required: a future step
        //                               with the same boundaries but a
        //                               different prefix must miss.
        //   - `cached_patch_reps`     — `[n_patches × patch_dim]`
        //                               post-latent patch reps.
        //   - `cached_seed_byte_reps` — `[max_seq_len × byte_dim]`
        //                               clone of the encoder's last-layer
        //                               byte_reps. Decoder consumes the
        //                               first `n_bytes × byte_dim` rows.
        //
        // The cache misses on the first step (cold) and on every step
        // where the fixed-stride patcher emits new boundaries — which
        // under the current placeholder patcher is every step. The
        // miss path runs the full forward; the hit path runs only the
        // decoder. Both paths terminate by sampling the last-position
        // logits, identical to [`Self::generate`].
        let mut cached_boundaries: Vec<usize> = Vec::new();
        let mut cached_tokens_prefix: Vec<u8> = Vec::new();
        let mut cached_patch_reps: Option<GpuVec> = None;
        let mut cached_seed_byte_reps: Option<GpuVec> = None;
        ...
        for step in 0..n_new_bytes {
            ...
            if tokens.len() > max_seq_len {
                let drop = tokens.len() - max_seq_len;
                tokens.drain(0..drop);
                // Window slid → cached prefix is no longer a prefix of
                // tokens. Invalidate.
                cached_boundaries.clear();
                cached_tokens_prefix.clear();
                cached_patch_reps = None;
                cached_seed_byte_reps = None;
            }
            let n = tokens.len();
            debug_assert!(n >= 1, "BltModel::generate_cached: empty token buffer");
            debug_assert!(n <= max_seq_len);

            // Same fixed-stride patcher as [`Self::generate`]. Bit-for-bit
            // boundary equality is required by the determinism contract.
            let stride = ((n + max_patches - 1) / max_patches).max(1);
            let mut boundaries: Vec<usize> = Vec::with_capacity(max_patches + 1);
            boundaries.push(0);
            let mut b = stride;
            ...
            let n_patches = boundaries.len().saturating_sub(1);
            debug_assert!(n_patches <= max_patches,
                "BltModel::generate_cached: too many patches ({} > {})",
                n_patches, max_patches);

            // Cache hit iff (a) same boundaries AND (b) same tokens.
            // Tokens are checked because the boundary array can match
            // by coincidence between unrelated prompts — the cached
            // patch_reps were computed from a specific byte sequence.
            let cache_hit = !cached_boundaries.is_empty()
                && cached_boundaries == boundaries
                && cached_tokens_prefix == tokens
                && cached_patch_reps.is_some()
                && cached_seed_byte_reps.is_some();

            // Per-step output buffer. `BltModel::forward` requires
            // `byte_logits.len() == n * 256` (debug-assert); allocate
            // once per step.
            let mut byte_logits = GpuVec::try_hip(n * 256)?;

            if cache_hit {
                cache_hits += 1;
                // Stage 3 only: decoder + reset of decoder KV cache.
                // The cached patch_reps + seed are bit-identical to
                // what [`Self::forward`] would have produced, so the
                // decoder's output is identical too.
                self.decoder.byte_kv_cache.reset();
                let cached_pr = cached_patch_reps.as_ref().unwrap();
                let cached_seed = cached_seed_byte_reps.as_ref().unwrap();
                self.decoder.forward(
                    batch,
                    cached_pr,
                    &boundaries,
                ...
                cache_misses += 1;
                // Full forward. We inline the body of [`Self::forward`]
                // here (rather than calling it) so we can capture the
                // intermediate post-latent patch_reps + seed byte_reps
                // into the cache without an extra pass.
                self.encoder.byte_kv_cache.reset();
                self.latent_kv_cache.reset();
                self.decoder.byte_kv_cache.reset();
                ...
                let mut patch_reps_view = GpuVec::try_hip(n_patches * patch_dim)?;
                self.encoder.forward(
                    batch, &tokens, &boundaries, &mut scratch.encoder, &mut patch_reps_view,
                )?;
                ...
                debug_assert_eq!(
                    patch_reps_buf.len(),
                    self.config.latent.max_patches * patch_dim,
                );
                run_latent_forward(
                    batch,
                    &self.latent,
                ...
                // Stage 3: decoder. Seed = encoder's last-layer byte_reps.
                self.decoder.forward(
                    batch,
                    &patch_reps_for_decoder,
                    &boundaries,
                ...
                // Populate the cache. We clone the post-latent patch
                // reps and the seed byte_reps so subsequent calls that
                // run a full `encoder.forward` (which mutates
                // `scratch.encoder.byte_reps`) don't trash the cache.
                let mut pr_clone = GpuVec::try_hip(n_patches * patch_dim)?;
                copy_d2d(&patch_reps_for_decoder, &mut pr_clone, n_patches * patch_dim)?;

                let seed_len = scratch.encoder.byte_reps.len();
                let mut seed_clone = GpuVec::try_hip(seed_len)?;
                copy_d2d(&scratch.encoder.byte_reps, &mut seed_clone, seed_len)?;

                cached_boundaries = boundaries.clone();
                cached_tokens_prefix = tokens.clone();
                cached_patch_reps = Some(pr_clone);
                cached_seed_byte_reps = Some(seed_clone);
            }

            // Read the LAST position's logits row. Identical to
            // [`Self::generate`] — bit-equality of the sampled byte
            // depends on bit-equality of this row.
            let mut all_logits = vec![0.0f32; n * 256];
            byte_logits.copy_to_host(&mut all_logits);
            row.copy_from_slice(&all_logits[(n - 1) * 256..n * 256]);

            // Sample. Inlined from [`Self::generate`] verbatim so the
            // RNG draw + greedy/temperature branches match bit-for-bit.
            let next_byte: u8 = if temperature == 0.0 {
                let mut best_i = 0usize;
                let mut best_v = row[0];
                for (i, &v) in row.iter().enumerate().skip(1) {
                ...
                debug_assert!(sum > 0.0,
                    "BltModel::generate_cached: softmax sum is zero");
                let inv_sum = 1.0f32 / (sum as f32);
                let u = lcg_uniform(rng_seed, step);
                let mut cdf = 0.0f32;
                let mut chosen = 255usize;
        ...
        // Diagnostic: cache hit/miss accounting. Logged via debug to
        // avoid spamming `--release` smoke tests; the example flips
        // `RUST_LOG` to surface it explicitly when the wall-clock
        // comparison runs.
        let _ = (cache_hits, cache_misses);
        #[cfg(debug_assertions)]
        eprintln!(
            "blt::generate_cached: hits={} misses={} (under fixed-stride patcher, \
             cache misses on every boundary shift; entropy patcher would lift this)",
...
mod tests {
    ...
    /// `generate_cached` must produce byte-identical output to the
    /// reference `generate` path under greedy decoding (temperature=0).
    /// The cache is just an amortization of intermediates — anything
    /// other than bit-equivalence implies a stale-cache bug.
    ///
    /// **Implementation note.** The encoder's cross-attn forward has a
    /// `debug_assert_eq!(byte_reps.len(), n_bytes * byte_dim)` in
    /// [`crate::cross_attn::CrossAttn::forward_encoder`] that fires
    /// when `n_bytes < max_seq_len`. (Pre-existing — predates this
    /// slice. The runtime path correctly only reads the first
    /// `n_bytes × byte_dim` rows; the assert's strict-equality is the
    /// bug. Out-of-scope to fix here.) To stay clear of it under
    /// `cargo test` (debug profile, asserts ON), we use a prompt
    /// length equal to `max_seq_len`. After the first step the
    /// window-slide in `generate*` keeps `tokens.len() == max_seq_len`,
    /// so the assert is satisfied throughout. The `--release` example
    /// (where debug_asserts are off) exercises the natural
    /// shorter-prompt path.
    #[test]
    fn generate_cached_matches_generate_greedy() {
        let _guard = modgrad_device::test_lock::hip_test_lock();
        if !runtime_available() {
            eprintln!("hip runtime unavailable, skipping");
            return;
        ...
        let cfg = tiny_config();
        let max_seq = cfg.encoder.max_seq_len;
        let mut model = BltModel::new(cfg.clone()).expect("BltModel::new");
        let mut scratch = BltScratch::new(&cfg).expect("BltScratch::new");
        let batch = HipBatch::new();

        // Length-`max_seq` prompt — see the doc-comment for why.
        let prompt: Vec<u8> = (0u8..max_seq as u8).collect();
        let n_new = 8usize;

        let naive = model
            .generate(&batch, &prompt, n_new, 0.0, 0xDEAD_BEEF, &mut scratch)
            .expect("generate (naive)");
        let cached = model
            .generate_cached(&batch, &prompt, n_new, 0.0, 0xDEAD_BEEF, &mut scratch)
            .expect("generate_cached");

        assert_eq!(
            naive, cached,
            "generate_cached must match generate bit-for-bit under greedy decoding",
        );
        assert_eq!(naive.len(), n_new);
    }
}
