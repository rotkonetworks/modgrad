...
fn main() {
    ...
    // ── WIDE-SMALL REBUILD ─────────────────────────────────────────────
    // Keep the EXACT small topology (8 regions, same names, same
    // connections, same d_input, NO router, AdaptiveGate) but rebuild each
    // region's CtmConfig with a bigger d_model. We CANNOT mutate rc.d_model
    // in place (CtmConfig::region derives synch/min_width/heads at
    // construction), so rebuild the whole region via CtmConfig::region,
    // copying the small config's exact signature and changing only d_model
    // (and memory proportionally). d_input stays small (16 cortical / 8 sub)
    // to avoid the cpu.rs matvec crash that eight_region_medium hits.
    //
    // Env knobs (defaults reproduce eight_region_small exactly):
    //   CORTEX_DM   d_model for input/attention/output/motor   (default 32)
    //   SUB_DM      d_model for cerebellum/basal_ganglia/insula/hippocampus (default 8)
    //   CORTEX_DIN  d_input  for cortical regions               (default 16)
    //   SUB_DIN     d_input  for subcortical regions            (default 8)
    //   CORTEX_MEM  memory_length cortical                      (default 8)
    //   SUB_MEM     memory_length subcortical                   (default 8 hipp / 4 others)
    let cortex_dm = env_usize("CORTEX_DM", 32);
    let sub_dm = env_usize("SUB_DM", 8);
    let cortex_din = env_usize("CORTEX_DIN", 16);
    let sub_din = env_usize("SUB_DIN", 8);
    let cortex_mem = env_usize("CORTEX_MEM", 8);
    let hipp_mem = env_usize("HIPP_MEM", 8);
    let sub_mem = env_usize("SUB_MEM", 4);
    {
        let ag = |beta: f32| ExitStrategy::AdaptiveGate { beta, threshold: 0.99 };
        // Mirror eight_region_small region-by-region. Signature:
        //   region(name, d_model, d_input, memory_length, deep_nlms, iterations, exit)
        // deep_nlms / beta per region copied verbatim from graph.rs.
        let regions = vec![
            CtmConfig::region("input",         cortex_dm, cortex_din, cortex_mem, false, ticks, ag(0.05)),
            CtmConfig::region("attention",     cortex_dm, cortex_din, cortex_mem, true,  ticks, ag(0.10)),
            CtmConfig::region("output",        cortex_dm, cortex_din, cortex_mem, true,  ticks, ag(0.10)),
            CtmConfig::region("motor",         cortex_dm, cortex_din, cortex_mem, false, ticks, ag(0.05)),
            CtmConfig::region("cerebellum",    sub_dm,    sub_din,    sub_mem,    false, ticks, ag(0.05)),
            CtmConfig::region("basal_ganglia", sub_dm,    sub_din,    sub_mem,    false, ticks, ag(0.10)),
            CtmConfig::region("insula",        sub_dm,    sub_din,    sub_mem,    false, ticks, ag(0.05)),
            CtmConfig::region("hippocampus",   sub_dm,    sub_din,    hipp_mem,   true,  ticks, ag(0.15)),
        ];
        let total_neurons: usize = regions.iter().map(|r| r.d_model).sum();
        config.regions = regions;
        config.n_global_sync = total_neurons.min(512);
        eprintln!(
            "WIDE rebuild: cortex_dm={cortex_dm} sub_dm={sub_dm} cortex_din={cortex_din} sub_din={sub_din} \
             total_neurons={total_neurons} n_global_sync={}",
    ...
    for b in 0..batches {
        ...
        let eval_n = env_usize("EVAL_N", 40);
        if b % eval_every == 0 || b == batches - 1 {
            let (mv, solve) = quick_eval(&w, eval_n, 9999);
    ...
    let (move_acc, solve_rate) = quick_eval(&w, env_usize("FINAL_N", 300), 12345);
    eprintln!(
